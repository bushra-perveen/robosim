...
public class DrawingPanel extends JPanel implements MouseMotionListener, MouseListener, AlgorithmListener {
	...
	public void paintComponent(Graphics g) {
		...
		g2.setColor(Color.RED);
		Rectangle2D rec = new Rectangle2D.Double(w.getStart().getyLoc() * pixelW, w.getStart().getxLoc() * pixelH, pixelW, pixelH);
		g2.fill(rec);
		g2.setColor(Color.GREEN);
	...
	public void smooth() {
		...
		if (path != null && path.size() > 0) {
			...
			sPath = GradientDescent.smooth(temp, weightData, weightSmooth, 0.0001, smoothingTimeout, smoothBoundryPoints);
		} else {
			sPath = null;
		}
	}
